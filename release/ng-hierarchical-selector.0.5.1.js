...
.directive('hierarchicalSelector', ['$compile', 'selectorUtils', function ($compile, selectorUtils) {
  return {
    ...
    controller: ['$scope', '$document', '$window', '$interpolate', function ($scope, $document, $window, $interpolate) {
      ...
      $scope.clearSelectedItems  = function(data) {
        for (var i = 0; i < data.length; i++){
          var itemMeta = selectorUtils.getMetaData(data[i]);
          itemMeta.selected = false;

          if (data[i].children){
            $scope.clearSelectedItems(data[i].children);
          }
        }      
      };

      $scope.$watch('clearSelected', function(newValue, oldValue) {
        $scope.selectedItems = [];
        $scope.clearSelectedItems($scope.data);
      });
